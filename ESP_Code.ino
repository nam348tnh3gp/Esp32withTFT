...
void RestartESP(String msg) {
  ...
  #if defined(DISPLAY_TFT_ST7789)
    tft_display_info("Restarting ESP...");
  #endif
...
namespace {
    ...
    #if defined(ESP32) && CORE == 2 && !defined(DISPLAY_TFT_ST7789)
    ...
    String httpGetString(String URL) {
        ...
        if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_MOVED_PERMANENTLY) {
            ...
            #endif
            #if defined(DISPLAY_TFT_ST7789)
              tft_display_info("Node fetch error");
            #endif
        }
        https.end();
        return payload;
    ...
    void SetupWifi() {
      ...
      #endif
      #if defined(DISPLAY_TFT_ST7789)
          tft_display_info("Waiting for node...");
      #endif
      SelectNode();
    }
...
// ============ HÀM HIỂN THỊ CHO TẤT CẢ CÁC LOẠI DISPLAY ============
void update_all_displays() {
    ...
    String uptime = "";
    if (uptime_days > 0) uptime += String(uptime_days) + "d";
    uptime += String(uptime_hours) + "h" + String(uptime_mins) + "m" + String(uptime_secs) + "s";
    
    float sharerate = (millisecs > 0) ? (share_count / (millisecs / 1000.0)) : 0;
    
    #if defined(DISPLAY_SSD1306) || defined(DISPLAY_16X2)
        display_mining_results(String(hashrate_float, 1), String(accepted_share_count), String(share_count), 
...
void setup() {
    ...
    job[0] = new MiningJob(0, configuration);
        ...
        #if defined(SERIAL_PRINTING)
          Serial.println("Initializing HSU07M sensor");
        #endif
    #endif
    ...
    WiFi.mode(WIFI_STA);
        ...
        String captivePortalHTML = R"(
          <title>Duino-Coin ESP32-S3 Miner</title>
          <style>
            body { background-color: #d4bff2; color: #363636; }
            button { background-color:#8645ef; border-radius:28px; border:1px solid #8645ef; cursor:pointer; color:#ffffff; padding:8px 16px; }
            input { border-radius:28px; border:1px solid #777777; padding:8px 16px; }
          </style>
        )";
        ...
        #endif
        #if defined(DISPLAY_TFT_ST7789)
          tft_display_info("Connecting to WiFi...");
        #endif
        SetupWifi();
    #endif
    ...
    SetupOTA();
      ...
      #if defined(SERIAL_PRINTING)
        #ifdef USE_LAN
          Serial.println("Dashboard: http://" + String(RIG_IDENTIFIER) + ".local (or http://" + ETH.localIP().toString() + ")");
        #else
          Serial.println("Dashboard: http://" + String(RIG_IDENTIFIER) + ".local (or http://" + WiFi.localIP().toString() + ")");
        #endif
      #endif
    ...
    // Hiển thị thông tin IP lên TFT
    #if defined(DISPLAY_TFT_ST7789)
        #ifdef USE_LAN
            tft_display_info("IP: " + ETH.localIP().toString());
        #else
            tft_display_info("IP: " + WiFi.localIP().toString());
        #endif
        delay(2000);
    #endif
...
void loop() {
  #if defined(ESP8266) || defined(CONFIG_FREERTOS_UNICORE) || defined(DISPLAY_TFT_ST7789)
    single_core_loop();
  #endif
  delay(10);
}
